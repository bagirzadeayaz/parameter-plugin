...
export async function submitPlatform(task, action, payload, fetchImpl = fetch) {
  ...
  const allowed = ['parametersAz', 'parametersRu', 'confidence', 'fieldEvidence', 'sources', 'productImages', 'unresolvedFields', 'notes', 'recoveryReport'];
  ...
  const response = await fetchImpl(ENDPOINT, {
    ...
    body: JSON.stringify({ data: { action, capability: task.platform.capability, productName: task.productName, category: task.category, ...(research ? { payload: research } : {}) } }),
  });
