...
Install the latest Kontakt Parameter plugin from https://github.com/bagirzadeayaz/parameter-plugin.git for my Codex account. This repository contains the plugin client; do not deploy a backend during installation. It submits future searches anonymously to the existing Kontakt platform endpoint.
...
Do not open a Parameter sign-in page or ask for an account, registration, password, refresh token, API key, or administrator approval. Do not configure direct database access. Future searches are stored anonymously through the existing platform endpoint, with local backups and local PDFs. Do not upload historical local searches.

Verify the installed version and anonymous platform-submission configuration, accurately report anything that remains incomplete, and tell me to start a new Codex chat so the plugin's tools and instructions load. In the new chat I can select Kontakt Parameter and send a product name. The plugin should follow its bundled research workflow and provide the product image, full parameter table with proof links beside confirmed values, and a locally generated PDF. Do not fabricate a successful test or start a product search without my product request.
...
Keep this installation conversation in English. The plugin's product-search prose must be Azerbaijani only; only parameter names and values should be paired in Azerbaijani in chat and PDF. Keep product names, website names, links, and exact parameter values intact.
